...
enum USBMaker {
    ...
    static func buildServerISO(projectRoot: String) async throws {
        ...
        let isoPath = root.appendingPathComponent("dist/vmnas-server-trixie-amd64.iso").path
        guard FileManager.default.fileExists(atPath: isoPath) else {
            throw NSError(domain: "USBMaker", code: 11, userInfo: [NSLocalizedDescriptionKey: "The build finished, but dist/vmnas-server-trixie-amd64.iso was not created."])
        }
    }
    ...
    static func createTerminalWriter(isoPath: String, extraIsoPaths: [String], device: USBDevice) async throws -> URL {
        ...
        let scriptsDirectory = FileManager.default
            ...
            .appendingPathComponent("Library/Application Support/VMnas/Scripts", isDirectory: true)
        try FileManager.default.createDirectory(at: scriptsDirectory, withIntermediateDirectories: true)
        let scriptURL = scriptsDirectory.appendingPathComponent("write-vmnas-installer-\(device.id).command")
        let shell = writeShell(
            source: source,
            extras: extraSources,
    ...
    private static func prepareWriteInputs(source: URL, extraIsoPaths: [String], device: USBDevice, persistent: Bool = false) throws -> PreparedWriteInputs {
        ...
        let stagingBase = persistent
            ? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support/VMnas/USBStaging", isDirectory: true)
        ...
        let stagingRoot = stagingBase
            .appendingPathComponent("vmnas-usb-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func writeShell(source stagedSource: URL, extras stagedExtras: [URL], manifestNames: [String], device: USBDevice, needsSudo: Bool, cleanupPath: String?) -> String {
        ...
        \(sudo)/usr/sbin/diskutil addPartition \(shellQuote(device.deviceNode)) ExFAT VMNAS_LOGS 512m
        ...
        echo "Creating VMNAS_ISOS data partition for guest media"
        \(sudo)/usr/sbin/diskutil unmountDisk force \(shellQuote(device.deviceNode)) || true
        \(sudo)/usr/sbin/diskutil addPartition \(shellQuote(device.deviceNode)) ExFAT VMNAS_ISOS 0b
        \(sudo)/usr/sbin/diskutil mountDisk \(shellQuote(device.deviceNode)) || true
        DEST="$(/usr/sbin/diskutil info -plist VMNAS_ISOS | /usr/bin/plutil -extract MountPoint raw -o - - 2>/dev/null || true)"
        if [ -z "$DEST" ]; then
          DEST="/Volumes/VMNAS_ISOS"
        fi
        if [ ! -d "$DEST" ]; then
          echo "VMNAS_ISOS data partition was not mounted." >&2
          exit 1
        fi
        echo "Copying guest media to $DEST/isos"
        mkdir -p "$DEST/isos"
        \(copyExtra)
        cat > "$DEST/vmnas-iso-library.txt" <<'EOF'
        \(manifest)
        EOF
        ...
        #!/bin/bash
        set -eu
        LOG_DIR="$HOME/Library/Logs/VMnas"
        ...
        echo "---- VMnas USB write started $(date) ----"
        ...
        \(cleanupScript)
        echo "---- VMnas USB write completed $(date) ----"
    ...
    private static func validateSpace(serverIso: URL, extraIsos: [URL], device: USBDevice) throws {
        ...
        guard required < device.sizeBytes else {
            throw NSError(domain: "USBMaker", code: 5, userInfo: [NSLocalizedDescriptionKey: "The USB drive is too small for the VMnas installer plus selected ISO library."])
        }
    }
    ...
    private static func usbWriteFailureMessage(fallback: String) -> String {
        let logURL = FileManager.default
            .homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Logs/VMnas/usb-maker.log")
        ...
        if tail.contains("Operation not permitted") {
            return """
            macOS blocked writing to the USB device. Give VMnas Admin Full Disk Access in System Settings > Privacy & Security > Full Disk Access, relaunch the app, then try again.
