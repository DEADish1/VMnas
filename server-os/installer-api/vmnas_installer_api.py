...
ROOT = Path("/opt/vmnas/installer-ui")
STATE = {
    "state": "ready",
    "phase": "Ready",
    "log": "",
...
PAIRING_PATH = Path("/var/lib/vmnas/pairing.json")
REPORT_LABEL = "VMNAS_LOGS"
...
def partition_plan(size_gb: float) -> dict:
    ...
    return {
        ...
        "data_label": "VMNAS-DATA",
        "data_mount": "/var/lib/vmnas/data",
...
def mounted_report_volume() -> Path | None:
    candidates = [
        ...
        Path(f"/run/media/vmnas/{REPORT_LABEL}"),
...
def write_error_report(exit_code: int, payload: dict) -> str:
    ...
    if volume is None:
        STATE["log"] += "\nNo VMNAS_LOGS partition was found for the install error report.\n"
        return ""
    ...
    if Path("/mnt/vmnas-target").exists():
        target_log_dir.mkdir(exist_ok=True)
        for source in [
            Path("/mnt/vmnas-target/etc/apt/sources.list"),
            Path("/mnt/vmnas-target/etc/apt/sources.list.d/proxmox.sources"),
            Path("/mnt/vmnas-target/etc/hosts"),
            Path("/mnt/vmnas-target/etc/hostname"),
            Path("/mnt/vmnas-target/var/lib/vmnas/install-profile.json"),
        ]:
            if source.exists():
...
def validate_install_payload(payload: dict) -> str | None:
    ...
    installer_script = Path("/usr/local/sbin/vmnas-install-proxmox")
    detected = hardware()
    try:
    ...
    if not installer_script.exists():
        return "Installer engine is missing from this VMnas boot media. Rebuild the server ISO and remake the USB."
    if not os.access(installer_script, os.X_OK):
        return "Installer engine is not executable on this VMnas boot media. Rebuild the server ISO and remake the USB."
    ...
    if not available[target_disk].get("plan", {}).get("installable", False):
        return "Selected disk is too small. VMnas needs at least 80 GB for the OS partition, update overhead, and a ready data partition."
    return None
...
def run_install(payload: dict) -> None:
    ...
    env = os.environ.copy()
    env["VMNAS_HOSTNAME"] = payload.get("hostname", "vmnas")
    env["VMNAS_ADMIN_USER"] = payload.get("admin_user", "vmnas")
    env["VMNAS_ADMIN_PASSWORD"] = payload.get("admin_password", "")
    env["VMNAS_MODULES"] = ",".join(payload.get("modules", []))
    detected = hardware()
    env["VMNAS_NAS_CPU_VCPUS"] = str(payload.get("nas_cpu_vcpus", detected["nas_default_vcpus"]))
    env["VMNAS_NAS_RAM_GB"] = str(payload.get("nas_ram_gb", detected["nas_default_memory_gb"]))
    env["VMNAS_TARGET_DISK"] = payload.get("target_disk", "")
    env["VMNAS_OS_SIZE_GB"] = str(DEFAULT_OS_SIZE_GB)
    env["VMNAS_MIN_OS_SIZE_GB"] = str(MIN_OS_SIZE_GB)
    env["VMNAS_ERASE_CONFIRMED"] = "1"
    set_phase("Partitioning selected server drive")
    set_phase("Installing Debian base")
    set_phase("Installing Proxmox and VMnas services")
    process = subprocess.Popen(
        ["/usr/local/sbin/vmnas-install-proxmox"],
    ...
    for line in process.stdout:
        if line.startswith("VMNAS_PHASE:"):
