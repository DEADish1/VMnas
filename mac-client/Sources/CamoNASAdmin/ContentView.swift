...
private struct CamoNASButtonStyle: ButtonStyle {
    var prominent = false
        ...
        configuration.label
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(configuration.isPressed ? CamoNASTheme.accentSoft : (prominent ? CamoNASTheme.accent : CamoNASTheme.panelRaised))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(configuration.isPressed ? CamoNASTheme.accentHot : CamoNASTheme.accent, lineWidth: 1)
            )
    }
}
...
struct ContentView: View {
    @StateObject private var api = CamoNASAPI()
    ...
    @State private var windowsLocalAccountName = "camonas"
    ...
    @State private var usbStatus = "Camo NAS will load the server ISO automatically when it is built."
    @State private var serverISOStatus = "Checking for Camo NAS server ISO..."
    ...
    private var serverISOCandidates: [String] {
        ...
        let legacyProjectRoot = "\(home)/Documents/\(["VM", "nas"].joined())"
        return [
            expectedServerISOPath,
            "\(legacyProjectRoot)/dist/camonas-server-trixie-amd64.iso",
            "\(legacyProjectRoot)/dist/\(["vm", "nas"].joined())-server-trixie-amd64.iso",
            "\(home)/Documents/CamoNAS/dist/camonas-server-trixie-amd64.iso",
            "\(FileManager.default.currentDirectoryPath)/dist/camonas-server-trixie-amd64.iso",
            "\(bundleResources)/camonas-server-trixie-amd64.iso"
        ].filter { !$0.isEmpty }
    }

    private var expectedServerISOPath: String {
        "\(projectRootCandidate)/dist/camonas-server-trixie-amd64.iso"
    }

    private var projectRootCandidate: String {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let brandedPath = "\(home)/Documents/CamoNAS"
        if FileManager.default.fileExists(atPath: brandedPath) {
            return brandedPath
        }
        let currentPath = "\(home)/Documents/\(["VM", "nas"].joined())"
    ...
    private func sidebarButton(_ section: AppSection) -> some View {
        ...
        return Button {
            ...
            Label(section.rawValue, systemImage: section.icon)
                .font(.callout.weight(.semibold))
                .foregroundStyle(isSelected ? Color.white : CamoNASTheme.textMuted)
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(.horizontal, 10)
                .padding(.vertical, 8)
                .background(
                    RoundedRectangle(cornerRadius: 7)
                        .fill(isSelected ? CamoNASTheme.accent : Color.clear)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 7)
                        .stroke(isSelected ? CamoNASTheme.accentHot.opacity(0.65) : Color.clear, lineWidth: 1)
                )
        }
        .buttonStyle(.plain)
    }
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                panel("Target VM") {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Text("This creates a test ISO and attaches it to the VM. Boot Windows, open the CAMONAS_WIN_TUNE disc, and run the script as Administrator to test the selected settings.")
                            .foregroundStyle(.secondary)
                    }
                }
                    ...
                    panel("Test Media") {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Text(result.scriptPreview)
                                ...
                                .padding(10)
                                .background(CamoNASTheme.field)
                                .clipShape(RoundedRectangle(cornerRadius: 8))
                                .overlay(
                                    RoundedRectangle(cornerRadius: 8)
                                        .stroke(CamoNASTheme.border.opacity(0.45), lineWidth: 1)
                                )
                        }
                    }
                }
    ...
    private var lockedModuleStore: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            panel("Server Required") {
                VStack(alignment: .leading, spacing: 12) {
                    Label("Pair this Mac with a running Camo NAS server before installing Apps.", systemImage: "lock.shield")
                        .font(.headline)
        ...
        panel("Pair Server") {
            VStack(alignment: .leading, spacing: 12) {
                Label(api.isServerReachable ? "Server found. Pair this Mac to continue." : "No paired server connection.", systemImage: api.isServerReachable ? "link.badge.plus" : "wifi.slash")
                    .font(.headline)
                Text("Hardware specs, VMs, OS downloads, benchmarks, and server modules stay hidden until this Mac is paired with the Camo NAS server.")
                    .foregroundStyle(.secondary)
                HStack {
    ...
    private var windowsTuningControls: some View {
        VStack(alignment: .leading, spacing: 12) {
            Toggle("Enable Camo NAS Windows tuning", isOn: $windowsTuningEnabled)
            ...
            Toggle("Disable OneDrive startup entry", isOn: $windowsDisableOneDriveStartup)
                .disabled(!windowsTuningEnabled)
            Text("Camo NAS generates a Windows answer-file/test ISO and PowerShell script. It avoids license bypasses and only changes the options selected here.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let device = selectedUSBDevice {
                ...
                if usbInstallerIsoSize == 0 {
                    Label("Required before writing: build the Camo NAS server ISO that boots first.", systemImage: "exclamationmark.circle")
                        .font(.caption)
                        .foregroundStyle(.orange)
                }
                ...
                if let remaining = usbEstimatedRemainingSize {
                    ...
                    Text("Estimate is based on selected file sizes plus Camo NAS library overhead before writing.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } else {
    ...
    private var usbMaker: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                panel("Flash Media Utility") {
                    VStack(alignment: .leading, spacing: 10) {
                        Label("Create bootable Camo NAS server installer media from this Mac.", systemImage: "externaldrive.badge.plus")
                            .font(.headline)
                        Text("This utility runs locally on the Mac and stays available before server pairing.")
                            .foregroundStyle(.secondary)
                        Text("The flash drive also gets a CAMONAS_LOGS area so failed server installs can save reports for troubleshooting.")
                            .foregroundStyle(.secondary)
                        ...
                        HStack {
                            ...
                            Button("Reveal Camo NAS Admin") {
                                revealCamoNASAdminInFinder()
                            }
                ...
                panel("Installer ISO") {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            TextField("Camo NAS server ISO", text: $usbIsoPath)
                                .textFieldStyle(.roundedBorder)
                            HStack {
                                Button("Auto Load") {
                        ...
                        if usbInstallerIsoSize > 0 {
                            ...
                            Text("Expected project output: \(projectRootCandidate)/dist/camonas-server-trixie-amd64.iso")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                ...
                panel("Server ISO Library") {
                    VStack(alignment: .leading, spacing: 10) {
                Text("Optional guest operating system images copied to the USB for the Camo NAS server to import after install.")
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
        ...
        .confirmationDialog("Erase USB drive?", isPresented: $showUSBConfirmation) {
            ...
            if let device = selectedUSBDevice {
                Text("This erases \(device.name) at \(device.deviceNode), writes the Camo NAS boot installer first, creates a CAMONAS_LOGS report area, then creates a CAMONAS_ISOS partition for \(usbExtraIsoURLs.count) selected guest media file(s). Estimated required: \(formatBytes(usbRequiredSize)). Estimated left: \(formatBytes(max(0, usbEstimatedRemainingSize ?? 0))).")
            } else {
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                permissionRow(
                    ...
                    detail: "Required for writing the Camo NAS installer directly to a flash drive. macOS will not add this entry automatically; open the pane, then use + or drag Camo NAS Admin from Finder into the list.",
                    icon: "lock.shield",
            ...
            Text("After enabling permissions, quit and reopen Camo NAS Admin so macOS applies the changes.")
                .font(.callout)
                .foregroundStyle(.secondary)
    ...
    private func permissionRow(title: String, detail: String, icon: String, actionTitle: String, action: @escaping () -> Void) -> some View {
        ...
        .padding(14)
        .background(CamoNASTheme.panelRaised, in: RoundedRectangle(cornerRadius: 8))
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .stroke(CamoNASTheme.border.opacity(0.45), lineWidth: 1)
        )
    ...
    private func currentWindowsTuningConfig() -> WindowsTuningConfig {
        return WindowsTuningConfig(
            ...
            localAccountName: windowsLocalAccountName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "camonas" : windowsLocalAccountName.trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func loadBundledServerISO() {
        if usbInstallerIsoSize > 0 {
            serverISOStatus = "Loaded Camo NAS server ISO: \(URL(fileURLWithPath: usbIsoPath).lastPathComponent)"
            return
        }
        if let path = serverISOCandidates.first(where: { fileSize(atPath: $0) > 0 }) {
            usbIsoPath = path
            serverISOStatus = "Loaded Camo NAS server ISO automatically."
            ...
            serverISOStatus = "Camo NAS server ISO path is loaded, but the ISO has not been built yet."
            if usbStatus == "Ready." || usbStatus.isEmpty {
                usbStatus = "Build the Camo NAS server ISO before making installer media."
            }
        }
    }
    ...
    private func buildServerISO() async {
        ...
        serverISOStatus = "Building Camo NAS server ISO..."
        usbStatus = "Building the Camo NAS server ISO. Docker must be installed and running on this Mac."
        do {
            try await USBMaker.buildServerISO(projectRoot: projectRootCandidate)
            usbIsoPath = "\(projectRootCandidate)/dist/camonas-server-trixie-amd64.iso"
            loadBundledServerISO()
        } catch {
    ...
    private func prepareUSBConfirmation() {
        ...
        guard usbInstallerIsoSize > 0 else {
            usbStatus = "The Camo NAS server ISO path is loaded, but the ISO file does not exist yet. Click Build Server ISO first."
            return
        }
    ...
    private func makeUSBInstaller() async {
        ...
        isWritingUSB = true
        usbStatus = "Writing Camo NAS boot installer first, then adding install reports and \(usbExtraIsoURLs.count) guest media file(s). macOS may ask for your password."
        do {
    ...
    private func metricTile(_ title: String, value: String, systemImage: String) -> some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(16)
        .background(CamoNASTheme.panelRaised)
        .clipShape(RoundedRectangle(cornerRadius: 8))
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .stroke(CamoNASTheme.border.opacity(0.55), lineWidth: 1)
        )
    }
    ...
    private var nasRecommendationText: String {
        guard let resources = api.resources else {
            return "Camo NAS will size the NAS preset after it detects the host CPU and memory."
        }
    ...
    private func statusBanner(_ message: String) -> some View {
        ...
        .padding(12)
        .background(CamoNASTheme.accentSoft.opacity(0.30))
        .clipShape(RoundedRectangle(cornerRadius: 8))
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .stroke(CamoNASTheme.accent.opacity(0.75), lineWidth: 1)
        )
    }
    ...
    private func liveVMOverlay(vm: VmSummary, url: URL) -> some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .stroke(CamoNASTheme.accent.opacity(0.70), lineWidth: 1)
        )
        .shadow(radius: 18)
    }
    ...
    private func tunedLiveURL(_ url: URL) -> URL {
        ...
        switch livePerformanceProfile {
        case "remoteAdaptive":
            set("camonas_profile", "remote-adaptive")
            set("fps_min", "60")
            set("fps", "120")
            set("quality", "7")
            set("compression", "2")
        case "balanced60":
            set("camonas_profile", "balanced-60")
            set("fps", "60")
            set("quality", "6")
            set("compression", "4")
        default:
            set("camonas_profile", "local-120")
            set("fps", "120")
            set("quality", "9")
            set("compression", "0")
        }
        ...
        Table(api.vms) {
            ...
            TableColumn("Actions") { vm in
                HStack(spacing: 6) {
                    ...
                    .help("Open live console in CamoNAS")
                    .disabled(vm.status != "running")
            ...
            HStack {
                ForEach(module.tags.prefix(3), id: \.self) { tag in
                    Text(tag)
                        ...
                        .padding(.vertical, 3)
                        .background(CamoNASTheme.accentSoft.opacity(0.32))
                        .clipShape(Capsule())
                }
            }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top) {
                ...
                Image(systemName: item.downloadSupported ? "arrow.down.circle" : "safari")
                    .foregroundStyle(item.downloadSupported ? CamoNASTheme.accentHot : .secondary)
            }
    ...
    private func revealCamoNASAdminInFinder() {
