...
if [ "$BUILD_STATUS" -ne 0 ] || [ ! -x "$CLIENT_DIR/.build/release/CamoNASAdmin" ]; then
  echo "Fresh Swift build failed; see $BUILD_LOG" >&2
  echo "Install the full Xcode toolchain, then rerun this script." >&2
  exit "${BUILD_STATUS:-1}"
fi

BINARY="$CLIENT_DIR/.build/release/CamoNASAdmin"
if [ -d "$APP_DIR" ]; then
  rm -rf "$APP_DIR"
fi
...
if [ -f "$ROOT/dist/camonas-server-trixie-amd64.iso" ]; then
  ditto --norsrc "$ROOT/dist/camonas-server-trixie-amd64.iso" "$RESOURCES_DIR/camonas-server-trixie-amd64.iso"
fi

if [ -f "$ROOT/dist/camonas-server-trixie-amd64.iso.sha256" ]; then
  ditto --norsrc "$ROOT/dist/camonas-server-trixie-amd64.iso.sha256" "$RESOURCES_DIR/camonas-server-trixie-amd64.iso.sha256"
fi
