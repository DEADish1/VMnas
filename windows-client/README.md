# VMnas Admin for Windows

This native Windows desktop client prepares the VMnas server installer USB.
...
Choose an existing `dist\vmnas-server-trixie-amd64.iso`, or use **Build ISO (WSL)**. The builder calls `server-os/build-iso.sh` inside WSL, which uses Docker Desktop. Select the intended USB device, type `ERASE`, and approve the Windows administrator prompt.
...
```powershell
.\tools\write-vmnas-usb.ps1 -DiskNumber 3 -IsoPath .\dist\vmnas-server-trixie-amd64.iso
```
