...
app = FastAPI(title="Camo NAS Agent", version="0.1.0")
proxmox = ProxmoxCli()
...
def require_paired_device(authorization: str = Header(default=""), x_camonas_token: str = Header(default="")) -> None:
    token = x_camonas_token.strip()
...
def discovery() -> DiscoveryInfo:
    urls = ["https://camonas.local:8765", "https://camonas:8765"]
...
def host_compatibility() -> HostCompatibility:
    ...
    workloads = [
        ...
        CompatibilityWorkload(
            ...
            details=[f"Camo NAS will cap sliders using detected {cpu} threads and {memory:g} GB RAM."],
        ),
        CompatibilityWorkload(
            id="containers",
            name="Containers",
...
def attach_windows_tuning_test_media(vmid: int, request: WindowsTuningConfig) -> WindowsTuningApplyResponse:
    try:
        ...
        return WindowsTuningApplyResponse(
            ...
            message="Windows tuning test media was attached. Start the VM, open the CAMONAS_WIN_TUNE disc, and run camonas-firstlogon.ps1 as Administrator to test the selected settings.",
        )
    except ProxmoxUnavailable as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    except Exception as exc:
