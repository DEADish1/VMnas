...
OS_CATALOG: list[dict] = [
    ...
    },
    {
        ...
        "details": "Official Valve SteamOS Deck Image is distributed as a compressed .img.bz2 after accepting Valve's SteamOS EULA. VMnas can import the downloaded .img.bz2 and create a VM from the decompressed disk image.",
        "version": "Latest",
        "size_mb": 3000,
        ...
        "download_supported": False,
        "install_notes": ["Open Valve's page, accept the EULA, then upload/import the .img.bz2 file.", "Use the Linux preset, UEFI boot, and GPU passthrough for serious gaming tests.", "This is a disk image, so VMnas imports it as the VM boot disk instead of attaching it as a CD-ROM installer."],
    },
    {
        "id": "bazzite-desktop",
...
class OsStore:
    ...
    def default(cls, iso_store: IsoStore) -> "OsStore":
        path = Path(os.environ.get("VMNAS_OS_STORE", "/var/lib/vmnas/os-downloads.json"))
        if not os.access(path.parent, os.W_OK):
            path = Path.home() / ".vmnas" / "os-downloads.json"
    ...
    def _download_iso(self, item: dict) -> None:
        ...
        try:
            ...
            request = urllib.request.Request(item["download_url"], headers={"User-Agent": "VMnas/0.9"})
