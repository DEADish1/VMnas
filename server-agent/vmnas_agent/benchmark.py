...
class BenchmarkStore:
    ...
    def default(cls) -> "BenchmarkStore":
        path = Path(os.environ.get("VMNAS_BENCHMARK_STORE", "/var/lib/vmnas/benchmark.json"))
        if not os.access(path.parent, os.W_OK):
            path = Path.home() / ".vmnas" / "benchmark.json"
...
def _disk_score() -> tuple[float, float]:
    root = Path(os.environ.get("VMNAS_BENCHMARK_TMP", "/var/lib/vmnas"))
    if not os.access(root, os.W_OK):
        root = Path.home() / ".vmnas"
