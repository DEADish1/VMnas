...
class ProxmoxCli:
    ...
    def list_vms(self) -> list[VmSummary]:
        if not self.available and os.environ.get("VMNAS_ALLOW_MOCK_VMS") == "1":
    ...
    def _mock_store_path(self) -> str:
        return os.environ.get("VMNAS_MOCK_VM_STORE", "/tmp/vmnas-mock-vms.json")
    ...
    def create_vm(self, request: VmCreateRequest) -> VmSummary:
        ...
        vm_storage = os.environ.get("VMNAS_VM_STORAGE", "local-lvm")
        args = [
            self.qm or "qm",
            "create",
            str(vmid),
        ...
        if boot_from_disk_image:
            if install_media is None or not install_media.exists():
                raise ValueError("Selected disk image does not exist on the VMnas server.")
        else:
    ...
    def access_links(self, vmid: int) -> VmAccessLinks:
        host = os.environ.get("VMNAS_PUBLIC_HOST", "SERVER-IP")
