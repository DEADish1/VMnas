...
$logDirectory = Join-Path $env:ProgramData 'VMnas'
...
try {
    ...
    if ($disk.LargestFreeExtent -ge 256MB) {
        ...
        Format-Volume -Partition $dataPartition -FileSystem exFAT -NewFileSystemLabel 'VMnas' -Confirm:$false | Out-Null
        Write-Host "Created VMnas data partition on drive $($dataPartition.DriveLetter):."
    }

    Write-Host 'VMnas installer USB is ready.'
}
catch {
