...
@MainActor
final class CamoNASAPI: ObservableObject {
    @Published var serverBaseURL = "https://camonas.local:8765"
    ...
    @Published var connectionMessage = "Click Find Server, then enter the pairing code shown on the Camo NAS server."
    ...
    var moduleStoreStatusText: String {
        if !isServerReachable {
            return "Connect to a Camo NAS server first."
        }
        if deviceToken == nil {
        ...
        return "Ready. Apps will install on the connected Camo NAS server."
    }
    ...
    private let session: URLSession = {
        let delegate = CamoNASURLSessionDelegate()
    ...
    func refresh() async {
        ...
        do {
            ...
            if !isDiscoveringServer {
                self.connectionMessage = "Could not reach a Camo NAS server. Click Find Server or type the server address from the Camo NAS screen."
            }
    ...
    func findServer() async {
        ...
        connectionMessage = "Looking for Camo NAS on this network..."
        errorMessage = nil
        ...
        clearAdminData()
        connectionMessage = "No Camo NAS server was found automatically. Make sure this Mac is on the same network, or type the address shown on the server."
    }
    ...
    func runBenchmark() async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before running the server benchmark."
            return
        }
        do {
    ...
    func performVMAction(vmid: Int, action: String) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before controlling VMs."
            return
        }
        do {
    ...
    func pair(deviceName: String, pin: String) async {
        do {
            ...
            self.connectionMessage = "Paired. This Mac can now control the Camo NAS server."
            await refresh()
        } catch {
            self.errorMessage = error.localizedDescription
    ...
    func installModule(id: String) async {
        guard isModuleStoreAvailable else {
            self.errorMessage = "Pair and connect to a Camo NAS server before installing server modules."
            return
        }
        do {
    ...
    func createVM(_ request: VmCreateRequest) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before creating VMs."
            return
        }
        do {
    ...
    func uploadIso(fileURL: URL) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before uploading installer media."
            return
        }
        do {
    ...
    func downloadOS(id: String) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before downloading operating systems to the server."
            return
        }
        do {
    ...
    func setRemoteAccess(enabled: Bool) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before changing remote access."
            return
        }
        do {
    ...
    func runServerUpdate() async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before updating server software."
            return
        }
        do {
    ...
    func deleteVM(vmid: Int) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before deleting VMs."
            return
        }
        do {
    ...
    func accessLinks(vmid: Int) async -> VmAccessLinks? {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before opening VM access."
            return nil
        }
        do {
    ...
    func createSnapshot(vmid: Int, name: String) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before creating snapshots."
            return
        }
        do {
    ...
    func attachWindowsTuningTestMedia(vmid: Int, config: WindowsTuningConfig) async {
        guard isConnected else {
            self.errorMessage = "Pair and connect to a Camo NAS server before testing Windows tuning settings."
            return
        }
        do {
    ...
    private func get<T: Decodable>(_ path: String) async throws -> T {
        ...
        if let http = response as? HTTPURLResponse, !(200..<300).contains(http.statusCode) {
            throw NSError(domain: "CamoNASAPI", code: http.statusCode, userInfo: [NSLocalizedDescriptionKey: "Server returned HTTP \(http.statusCode)."])
        }
    ...
    private func uploadFile<T: Decodable>(_ path: String, fileURL: URL, fieldName: String) async throws -> T {
        ...
        let boundary = "CamoNASBoundary-\(UUID().uuidString)"
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
    ...
    private func addAuth(to request: inout URLRequest) {
        ...
        if let token = deviceToken, !token.isEmpty {
            ...
            request.setValue(token, forHTTPHeaderField: "X-CamoNAS-Token")
        }
    }
...
final class CamoNASURLSessionDelegate: NSObject, URLSessionDelegate {
    func urlSession(
